...
pub struct Runner<S: StoreProvider> {
    ...
    add_apps: Vec<(String, Box<dyn Fn(Client<S>) -> Application>)>,
}
...
impl<S: StoreProvider + Clone> Runner<S> {
    ...
    {
        self.add_apps.push((id.into(), Box::new(f)));
        self
    }
        ...
        A: ctaphid_dispatch::app::App + 'static,
    {
        self.add_apps.push((
            id.into(),
            Box::new(move |client| Application::Ctaphid(Box::new(f(client)))),
        ));
        self
    }
    ...
    fn create_apps(&self, service: &mut Rc<RefCell<Service<Platform<S>>>>) -> Vec<Application> {
        ...
        for (name, app) in &self.add_apps {
            let client = service
                .borrow_mut()
                .try_new_client(name, Syscall::from(service.clone()))
                .expect("failed to create client");
            apps.push(app(client));
        }
        apps
    }
}
